...
test('does not add the Windows publisher constraint to macOS updates', async () => {
  const appOutDir = await mkdtemp(path.join(tmpdir(), 'notestack-mac-app-out-'))
  ...
  try {
    ...
    await writeAppUpdatePublisher(appOutDir, builderConfigPath, 'darwin')

    const appUpdate = YAML.parse(await readFile(appUpdatePath, 'utf8'))

    assert.equal(appUpdate.publisherName, undefined)
    assert.equal(appUpdate.repo, 'card-notes-app')
  } finally {
    await rm(appOutDir, { force: true, recursive: true })
  }
