...
import { createNote, deleteNote, getNotes, updateNote } from '../../../api/notes/requests';
...
describe('useNotesQuery', () => {
  ...
  it('keys notes lists by sort query', () => {
    expect(notesQueryKeys.list({ sortBy: 'createdAt', sortDirection: 'desc' })).not.toEqual(
...
describe('notes mutation hooks', () => {
  ...
  it('creates notes and invalidates notes lists', async () => {
    ...
    await act(async () => {
      await expect(result.current.mutateAsync(payload)).resolves.toEqual(createdNote);
    });
  ...
  it('updates notes and invalidates notes lists', async () => {
    ...
    await act(async () => {
      await expect(result.current.mutateAsync({ id: 'note-1', note: payload })).resolves.toEqual(updatedNote);
    });
