...
describe('settings mutations', () => {
  ...
  it('creates columns and invalidates the columns query', async () => {
    ...
    await act(async () => {
      await expect(result.current.mutateAsync(payload)).resolves.toEqual(createdColumn);
    });

    expect(createColumn).toHaveBeenCalledWith(payload);
    expect(invalidateQueries).toHaveBeenCalledWith({ queryKey: settingsQueryKeys.columns() });
  });
  ...
  it('updates columns and invalidates the columns query', async () => {
    ...
    await act(async () => {
      await expect(result.current.mutateAsync({ column: payload, id: 'column-1' })).resolves.toEqual(updatedColumn);
    });
  ...
  it('reorders columns and invalidates the columns query', async () => {
    ...
    const reorderedColumns = [createColumnDto('column-1'), createColumnDto('column-2')];
    vi.mocked(reorderColumns).mockResolvedValue(createResponse(reorderedColumns));
    const queryClient = createTestQueryClient();
    const invalidateQueries = vi.spyOn(queryClient, 'invalidateQueries');
    ...
    await act(async () => {
      await expect(result.current.mutateAsync(payload)).resolves.toEqual(reorderedColumns);
    });
  ...
  it('deletes columns and invalidates the columns query', async () => {
    ...
    expect(deleteColumn).toHaveBeenCalledWith('column-1', { deleteMode: 'definitionOnly' });
    expect(invalidateQueries).toHaveBeenCalledWith({ queryKey: settingsQueryKeys.columns() });
  });
  ...
  it('deletes columns with values and invalidates columns and notes queries', async () => {
    ...
    expect(deleteColumn).toHaveBeenCalledWith('column-1', { deleteMode: 'definitionAndValues' });
    expect(invalidateQueries).toHaveBeenCalledWith({ queryKey: settingsQueryKeys.columns() });
    expect(invalidateQueries).toHaveBeenCalledWith({ queryKey: notesQueryKeys.lists() });
  });
  ...
  it('updates general settings and invalidates the general settings query', async () => {
    ...
    vi.mocked(updateGeneralSettings).mockResolvedValue(createResponse(updatedSettings));
    const queryClient = createTestQueryClient();
    const invalidateQueries = vi.spyOn(queryClient, 'invalidateQueries');
    ...
    await act(async () => {
      await expect(result.current.mutateAsync(payload)).resolves.toEqual(updatedSettings);
    });

    expect(updateGeneralSettings).toHaveBeenCalledWith(payload);
    expect(invalidateQueries).toHaveBeenCalledWith({ queryKey: settingsQueryKeys.general() });
  });
  ...
  it('imports app data and invalidates settings and notes queries', async () => {
    ...
    await act(async () => {
      await expect(result.current.mutateAsync(payload)).resolves.toEqual(importResult);
    });
