...
export const useUpdateGeneralSettingsMutation = () => {
  ...
  return useMutation({
    mutationFn: (settings: UpdateGeneralSettingsDto) => updateGeneralSettings(settings).then((response) => response.data),
    onSuccess: () => {
      return queryClient.invalidateQueries({ queryKey: settingsQueryKeys.general() });
    },
  });
};
