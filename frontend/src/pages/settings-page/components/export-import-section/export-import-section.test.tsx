...
describe('ExportImportSection', () => {
  it('exports app data and starts a JSON download', async () => {
    ...
    expect(createdAnchor?.download).toBe('card-notes-export-2026-07-08T10-00-00-000Z.json');
  ...
  it('shows export failure feedback when exporting rejects', async () => {
    ...
    expect(await screen.findByText('App data could not be exported right now.')).toBeTruthy();
  });
  ...
  it('imports a valid JSON export file and shows the result summary', async () => {
    ...
    fireEvent.change(screen.getByLabelText('JSON import file'), {
      target: {
        files: [new File([JSON.stringify(exportData)], 'card-notes-backup.json', { type: 'application/json' })],
      },
    });
    ...
    expect(screen.getByText('Select a JSON export file to import. Existing notes stay in place and imported notes are appended.')).toBeTruthy();
  });
  ...
  it('shows a validation error when the selected file does not match the export format', async () => {
    ...
    fireEvent.change(screen.getByLabelText('JSON import file'), {
      target: {
        files: [new File([JSON.stringify({ version: 1 })], 'partial.json', { type: 'application/json' })],
      },
    });
    ...
    expect(
      await screen.findByText('The selected file does not match the expected Card Notes export format.'),
    ).toBeTruthy();
    expect(importMutation.mutateAsync).not.toHaveBeenCalled();
  });
  ...
  it('shows import failure feedback when the backend rejects the payload', async () => {
    ...
    expect(await screen.findByText('The selected data could not be imported right now.')).toBeTruthy();
  });
