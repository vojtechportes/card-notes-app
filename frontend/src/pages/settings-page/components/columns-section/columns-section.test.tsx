...
vi.mock('@dnd-kit/core', async () => {
  ...
  return {
    ...
    DndContext: ({ children, onDragEnd }: { children: ReactNode; onDragEnd?: (event: unknown) => void }) => (
      <div>
        <button
          onClick={() => onDragEnd?.({ active: { id: 'reference-link' }, over: { id: 'summary' } })}
          type="button"
        >
          Trigger reorder
        </button>
...
describe('ColumnsSection', () => {
  it('renders loaded columns and does not expose delete for default columns', () => {
    ...
    expect(within(defaultRow as HTMLElement).queryByRole('button', { name: 'Delete column' })).toBeNull();
    expect(within(customRow as HTMLElement).getByRole('button', { name: 'Delete column' })).toBeTruthy();
  });
  ...
  it('edits a column and keeps default identity fields disabled', async () => {
    ...
    fireEvent.click(within(screen.getByText('Created at').closest('li') as HTMLElement).getByRole('button', { name: 'Edit column' }));

    expect((screen.getByLabelText('Column name') as HTMLInputElement).disabled).toBe(true);
    expect(screen.getByRole('combobox', { name: 'Column type' }).getAttribute('aria-disabled')).toBe('true');
  ...
  it('toggles visibility and reorders columns', async () => {
    ...
    fireEvent.click(within(hiddenRow).getByRole('button', { name: 'Show column' }));
  ...
  it('shows loading and error states from the columns query', () => {
    ...
    renderColumnsSection();
    expect(screen.getByText('Columns could not be loaded right now.')).toBeTruthy();
  });
});
