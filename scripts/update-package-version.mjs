...
const selectedFlags = process.argv.slice(2).filter((argument) => bumpFlags.has(argument));
...
const parseVersion = (version, filePath) => {
  ...
  if (!match) {
    throw new Error(`${filePath} has unsupported version "${version}". Expected x.y.z.`);
  }
...
try {
  const packageJsonFiles = await Promise.all(packageJsonPaths.map(readPackageJson));
  const currentVersion = packageJsonFiles[0].version;
  const mismatchedVersion = packageJsonFiles.find(({ version }) => version !== currentVersion);
  ...
  await Promise.all(
    packageJsonFiles.map(({ fullPath, packageJson }) => {
      ...
      return writeFile(fullPath, `${JSON.stringify(packageJson, null, 2)}\n`, 'utf8');
    }),
  );

  console.log(`Updated package versions from ${currentVersion} to ${nextVersion}.`);
} catch (error) {
