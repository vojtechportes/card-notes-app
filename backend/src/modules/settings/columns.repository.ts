...
export class ColumnsRepository {
  ...
  ensureDefaultColumns(defaultColumns: DefaultNoteColumn[]): void {
    ...
    const seedDefaultColumns = database.transaction((columns: DefaultNoteColumn[]) => {
  ...
  findById(id: string): NoteColumn | undefined {
    const row = this.getDatabase().prepare('SELECT * FROM note_columns WHERE id = ?').get(id) as NoteColumnRow | undefined;

    return row ? this.mapColumnRow(row) : undefined;
  }

  findByName(name: string): NoteColumn | undefined {
    const row = this.getDatabase().prepare('SELECT * FROM note_columns WHERE name = ?').get(name) as NoteColumnRow | undefined;
  ...
  updateSortOrders(columnIds: string[]): void {
    const database = this.getDatabase();
    const updateSortOrder = database.prepare('UPDATE note_columns SET sort_order = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?');
