...
import { assertOneDriveGraphUrl } from './utils/assert-one-drive-graph-url.util'
import { assertOneDriveUploadUrl } from './utils/assert-one-drive-upload-url.util'
...
export class OneDriveFileService {
  ...
  async listChildren(
    ...
    const response = await this.httpClient.request(assertOneDriveGraphUrl(url))
  ...
  async createResumableFile(
    ...
    return this.uploadChunks(assertOneDriveUploadUrl(session.uploadUrl), bytes)
  }

  async updateResumableFile(
    itemId: string,
    name: string,
    bytes: Buffer,
    expectedVersion: string
    ...
    const sessionResponse = await this.httpClient.request(
      `${ONE_DRIVE_GRAPH_BASE_URL}/me/drive/items/${encodeURIComponent(itemId)}/createUploadSession`,
      {
        method: 'POST',
        headers: {
