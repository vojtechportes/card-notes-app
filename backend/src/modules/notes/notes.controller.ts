...
export class NotesController {
  ...
  createNote(@Body() body: CreateNoteDto = {}): NoteDto {
    return this.notesService.createNote({ values: this.resolveNoteValues(body) });
  }

  @Get()
  @ApiOperation({ summary: 'List notes' })
  @ApiOkResponse({ description: 'Notes sorted by the requested timestamp field.', type: NoteDto, isArray: true })
  ...
  private resolveNoteValuePatch(body: UpdateNoteDto): NoteValuePatch | undefined {
    this.ensureRequestBodyIsRecord(body);
  ...
  private ensureValuesAreRecord(values: unknown): void {
    if (!this.isRecord(values)) {
      throw new BadRequestException('Note values must be an object keyed by column id.');
    }
  }
  ...
  private resolveSortOptions(query: ListNotesQueryDto = {}): NoteSortOptions {
    ...
    const sortDirection = this.normalizeSortDirection(query.sortDirection ?? NoteSortDirectionEnum.Desc);
  ...
  private normalizeSortDirection(sortDirection: NoteSortDirectionEnum): NoteSortDirectionEnum {
