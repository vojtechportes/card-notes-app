...
export class NotesService {
  ...
  updateNote(id: string, input: UpdateNoteInput): Note {
    ...
    const note = this.notesRepository.updateValues(id, values, this.createTimestamp());
  ...
  private ensureValuesAreValid(values: NoteValues): void {
    for (const value of Object.values(values)) {
      if (value === null) {
        throw new BadRequestException('Note values cannot be null when creating a note.');
      }
    }
  ...
  private ensureValuePatchIsValid(values: NoteValuePatch): void {
    ...
    for (const [columnId, value] of Object.entries(values)) {
      ...
      if (this.isSystemTimestampColumn(column)) {
        throw new BadRequestException('Note timestamps are managed by the system.');
      }
