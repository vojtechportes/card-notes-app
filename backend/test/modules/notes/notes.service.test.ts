...
describe(NotesService.name, () => {
  ...
  it('round-trips supported text, date, number, link, and image values', () => {
    ...
    const dateColumn = settingsService.createColumn({
      name: 'reviewedOn',
      title: 'Reviewed on',
      type: ColumnTypeEnum.Date,
    })
    const numberColumn = settingsService.createColumn({
      name: 'rating',
      title: 'Rating',
      type: ColumnTypeEnum.Number,
    })
    const linkColumn = settingsService.createColumn({
      name: 'sourceUrl',
      title: 'Source URL',
  ...
  it('deletes all notes, cascades their values, and preserves settings', () => {
    ...
    expect(settingsService.listColumns().map((column) => column.id)).toContain(
      summaryColumn.id
    )
    expect(
      databaseService
