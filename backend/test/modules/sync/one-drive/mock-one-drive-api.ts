...
export class MockOneDriveApi {
  ...
  private async handleFetch(
    ...
    if (url.pathname.endsWith('/createUploadSession')) {
      return this.handleUploadSession(init)
    }
  ...
  private handleUploadSession(init: RequestInit): Response {
    ...
    return this.jsonResponse({ uploadUrl: `https://upload.test/${sessionId}` })
  }
  ...
  private handleUpload(url: URL, init: RequestInit): Response {
    ...
    const stored = this.storeFile(session.name, session.bytes)
