...
describe('OneDriveSyncProviderAdapter', () => {
  it('uses special/approot and exposes only the app-folder permission', async () => {
    ...
    expect(
      api.requests.every(({ init, url }) => {
        if (new URL(url).hostname === 'notestack-test.up.1drv.com') {
  ...
  it('rejects untrusted delta and pagination links before authorization', async () => {
    const api = new MockOneDriveApi()
    const adapter = createAdapter(api)

    await expect(
      adapter.listChanges('https://attacker.example/v1.0/me/drive/delta')
    ...
    api.queueResponse(
      ...
      new Response(
        JSON.stringify({
          value: [],
          '@odata.nextLink':
            'https://attacker.example/v1.0/me/drive/items/app-root/delta',
        }),
        { status: 200 }
      )
    )
    const firstPage = await adapter.enumerateObjects()

    await expect(
      adapter.enumerateObjects(firstPage.nextPageToken!)
    ).rejects.toMatchObject({ kind: SyncProviderErrorKindEnum.Permanent })
    expect(
      api.requests.some(
        ({ url }) => new URL(url).hostname === 'attacker.example'
      )
    ).toBe(false)
  })
  ...
  it('rejects untrusted upload-session URLs before sending asset bytes', async () => {
    const untrustedUrls = [
      'http://127.0.0.1:3000/internal',
      'https://attacker.example/upload',
      'https://user:password@tenant.sharepoint.com/upload',
    ]

    for (const uploadUrl of untrustedUrls) {
      const api = new MockOneDriveApi()
      const adapter = createAdapter(api, emptyMappingReader, 1)
      const bytes = Buffer.alloc(700_000, 6)
      const hash = createHash('sha256').update(bytes).digest('hex')
      api.queueResponse(
        '/createUploadSession',
        new Response(JSON.stringify({ uploadUrl }), { status: 200 })
      )

      await expect(
        adapter.createAsset(`assets/${hash}.bin`, bytes, hash)
      ).rejects.toMatchObject({ kind: SyncProviderErrorKindEnum.Permanent })
      expect(api.requests.some(({ url }) => url === uploadUrl)).toBe(false)
    }
  })
  ...
  it('resumes interrupted upload sessions and verifies asset bytes', async () => {
    ...
    expect(
      api.requests.some(
        ({ init, url }) =>
          new URL(url).hostname === 'notestack-test.up.1drv.com' &&
          init.method === 'GET'
      )
    ).toBe(true)
  })

  it('resumably updates and verifies large existing assets conditionally', async () => {
    const api = new MockOneDriveApi()
    const adapter = createAdapter(api, emptyMappingReader, 1)
    const bytes = Buffer.alloc(700_000, 11)
    const hash = createHash('sha256').update(bytes).digest('hex')
    const logicalKey = `assets/${hash}.bin`
    const created = await adapter.createAsset(logicalKey, bytes, hash)
    api.queueResponse(
      'notestack-test.up.1drv.com',
      new Response('{}', { status: 503 })
    )
    ...
    expect(updated.providerVersion).not.toBe(created.providerVersion)
    expect((await adapter.readObject(logicalKey)).bytes).toEqual(bytes)
    expect(
      api.requests.some(
        ({ init, url }) =>
          init.method === 'POST' &&
          url.includes(
            `/items/${created.providerObjectId}/createUploadSession`
          ) &&
          new Headers(init.headers).get('if-match') === created.providerVersion
      )
    ).toBe(true)
    expect(
      api.requests.some(
  ...
  it('derives a stable least-privilege identity and lists the app-root workspace', async () => {
    const api = new MockOneDriveApi()
    ...
    await expect(adapter.getIdentity()).resolves.toMatchObject({
      accountId: 'microsoft-drive',
      accountDisplayName: null,
    })
    await expect(adapter.listWorkspaces()).resolves.toEqual([])
    await adapter.createWorkspace(workspaceId)

    const restarted = new OneDriveSyncProviderAdapter({
      accessTokenProvider: () => 'token',
      objectMappingReader: emptyMappingReader,
      fetch: api.fetch,
    })

    await expect(restarted.listWorkspaces()).resolves.toEqual([
      expect.objectContaining({ providerWorkspaceId: workspaceId }),
    ])
  })

  it('conditionally repairs and verifies an existing asset', async () => {
    const api = new MockOneDriveApi()
    const adapter = createAdapter(api)
    const bytes = Buffer.from('verified repair bytes')
    const hash = createHash('sha256').update(bytes).digest('hex')
    const logicalKey = `assets/${hash}.bin`
    const created = await adapter.createAsset(logicalKey, bytes, hash)

    const repaired = await adapter.updateAsset(
      logicalKey,
      bytes,
      hash,
      created.providerVersion
    ...
    expect(repaired.providerVersion).not.toBe(created.providerVersion)
    expect((await adapter.readObject(logicalKey)).bytes).toEqual(bytes)
    expect(
      api.requests.some(
        ({ init, url }) =>
...
describe('OneDriveHttpClient error classification', () => {
  ...
  for (const [status, code] of [
    [410, 'resyncRequired'],
    [404, 'syncStateNotFound'],
    [404, 'invalidDeltaToken'],
  ] as const) {
    it(`maps ${status} ${code} delta state to InvalidCursor`, async () => {
      const api = new MockOneDriveApi()
      api.queueResponse(
        '/delta',
