...
- [ ] TMSC-42. Add signed and notarized macOS releases
  ...
  - Use configuration supported by the repository's pinned Electron Builder version. Add a macOS `app` build with a drag-install DMG for manual installation and a ZIP update payload containing the fully signed/notarized `.app`, plus the `latest-mac.yml` metadata required by `electron-updater`; use platform-, version-, and architecture-safe artifact names so Windows and macOS assets and parallel architecture jobs cannot overwrite each other.
  - Add a branded `.icns` icon derived from the existing NoteStack source artwork, use an appropriate macOS application category, and explicitly record the minimum supported macOS version. Do not upgrade Electron solely as part of this task unless the chosen OS policy cannot be met safely by the pinned version and that scope change is approved first.
  - Support both Apple Silicon (`arm64`) and Intel (`x64`) Macs by default. A narrower architecture promise requires an explicit product decision recorded in this task and in release documentation before implementation. Prefer independently built architecture artifacts for the first release; use a universal binary only if native-module packaging and updates are proven for both architectures.
  - Build every promised architecture on a macOS GitHub Actions runner. Stage and rebuild `better-sqlite3` for the matching Electron version, target OS, and target architecture; never reuse `.backend-runtime`, `win-unpacked`, or native modules produced by a Windows or different-architecture job.
  - Define and test one authoritative multi-architecture updater design for the pinned `electron-updater` version. Produce one merged `latest-mac.yml` containing both architecture-specific ZIP entries, include `${arch}` in macOS artifact names and the literal `arm64` marker required by the pinned `MacUpdater` selection behavior, and ensure architecture jobs cannot clobber the manifest. Validate the merger, hashes, manifest entries, filename markers, and actual Mach-O architecture so filename matching is never the only proof; add focused coverage for the pinned updater's architecture selection or an equivalent packaged integration test.
  - Sign the complete `.app` bundle and all nested executable/native content with an Apple `Developer ID Application` identity, enable hardened runtime, and add only the entitlements actually required by Electron/NoteStack. Submit the selected distributable to Apple's notary service, staple the accepted ticket to supported formats such as the `.app` and DMG, and verify signing, Gatekeeper assessment, notarization, and stapling before placing the signed/notarized `.app` in the ZIP update payload.
  - Keep the Apple certificate/private key and notarization credentials in GitHub secrets. Prefer App Store Connect API-key authentication for unattended notarization, scope credentials to the macOS release job, fail fast with actionable non-secret diagnostics when required configuration is absent, and never print certificate material, passwords, tokens, OAuth secrets, or API-key contents. Ordinary builds and unsigned local packaging checks must remain usable without release credentials, but must not publish public macOS assets.
  - Make `electron/scripts/write-app-update-publisher.mjs` platform-aware: retain the Certum publisher-name constraint only for Windows, preserve the Windows update-code-signature verifier, and allow Electron Updater to use its signed macOS update path without Windows-only metadata.
  - Generalize `electron/scripts/verify-packaged-backend.mjs` and `electron/scripts/verify-packaged-oauth-flow.mjs` to resolve both Windows unpacked layouts and macOS `.app/Contents` executable/resource layouts. On each promised Mac architecture, continue verifying the production dependency closure, the target-native `better_sqlite3.node`, backend health/startup, and concurrent isolated packaged OAuth verification rather than skipping those gates.
  - Generalize release-artifact validation and publishing without weakening Windows checks. Validate the platform-specific manifest schema, version, URLs, hashes, sizes, update payloads, DMG presence, and architecture mapping before upload; after upload, query the GitHub release and prove every manifest-referenced asset exists exactly once and refers to the released version and correct architecture.
  - Preserve the existing GitHub-release version-update commit as the source for all platform jobs. Upload intermediate CI artifacts under collision-safe names, then use one final publish job that depends on successful build, signing, notarization, and validation for every promised platform; that job must download the complete validated set and publish Windows and macOS assets together before performing post-upload verification. A failed prerequisite must expose no new public platform assets. Because GitHub asset uploads are not transactional, implement narrowly scoped rollback or recovery for mid-upload/post-verification failures so an incomplete new asset set is never treated as available. Make reruns idempotent by replacing only the exact expected assets for that version, never by broad or glob-based release-asset deletion.
  - Freeze and verify application identity continuity across Windows and macOS releases: preserve `appId`/bundle identifier `com.cardnotes.app`, `productName` `NoteStack`, the GitHub owner/repository and update channel, and the existing `card-notes-app` data subdirectory below Electron `appData`. Any identity change requires an explicit migration/product decision before the first Mac release. Preserve safe-storage/Keychain credentials, OAuth callbacks, external-link handling, updater/prerelease preferences, single-instance behavior, close-all behavior, Dock `activate` window recreation, and macOS data discovery under `~/Library/Application Support`; test upgrade continuity rather than validating only fresh installs. Verify the current frameless Windows-style title bar on macOS and make only the platform-specific shell/UI changes needed for usable, accessible Mac window controls; a broad title-bar redesign is out of scope.
  - Update `readme.md` with supported Mac architectures and minimum OS, explicit local Windows/macOS packaging commands, the distinction between unsigned local checks and publishable releases, Apple Developer prerequisites, required GitHub secret names and safe setup guidance, expected DMG/ZIP/manifest assets, release troubleshooting, and clean-machine install/update verification.
  - Add focused tests for platform path resolution, configuration branching, artifact selection, manifest validation, missing-secret failure, and workflow wiring where practical. Run Prettier on every modified file, then run the full Electron tests/lint/build and root build plus existing Windows directory/NSIS/release regression gates.
  - On matching `arm64` and `x64` macOS runners or hardware, verify `.app` executable/resource locations, native SQLite loading, packaged backend health, OAuth verification, DMG mount/copy/launch, ZIP update payload integrity, and `latest-mac.yml` selection. Run `codesign --verify --deep --strict`, Gatekeeper assessment, Apple notarization-status validation, and stapling validation before publishing.
  - Before the first public Mac release, perform a two-version update rehearsal on every promised architecture using private/draft fixture releases or two prereleases, with explicit cleanup/retention rules. Manually smoke-test a clean standard-user installation without Gatekeeper bypass, first launch, quit/reopen and Dock reactivation, note CRUD, images, import/export, persistence, OAuth connections, synchronization startup, external links, custom window controls, and an in-app update from the older signed/notarized rehearsal build. Re-run a Windows install and update smoke test to demonstrate no regression.
  - Record the chosen architecture/manifest strategy, minimum macOS version, signing identity type, notarization authentication method, CI runner coverage, and evidence from the first successful signed/notarized GitHub release in the task or release documentation. App Store distribution/sandboxing, Linux packaging, replacing GitHub Releases, and unrelated UI redesigns remain out of scope.
